{ pkgs, lib, ... }:
{
  home.shellAliases = {
    ...
    "wt" =
      "if [ -f Cargo.toml ]; then mv Cargo.toml _Cargo.toml; echo Deactivated; elif [ -f _Cargo.toml ]; then mv _Cargo.toml Cargo.toml; echo Activated; else; echo Theres nothing here; fi";
  };
  programs.zsh = {
    shellAliases = {
